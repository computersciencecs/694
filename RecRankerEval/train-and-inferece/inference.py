# This file is used to perform inference using the fine-tuned LLM.
import argparse
import json
import re
import jsonlines
...
def load_finetuned_model(model_name, adapter_dir, token):
...
def load_and_merge_model(model_name, adapter_dir, token, merged_model_dir='./merged_model'):
    # Load the base model and tokenizer
    model = AutoModelForCausalLM.from_pretrained(model_name, torch_dtype=torch.float16, token=token)
    tokenizer = AutoTokenizer.from_pretrained(model_name, token=token)
...
if __name__ == "__main__":
    ...
    merged_model_dir = load_and_merge_model(args.model, args.adapter_dir, args.token)
